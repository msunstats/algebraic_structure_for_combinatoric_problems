# --- bo_method.R: Bayesian Optimization (BO) Method for Patient Subgroup Discovery ---
...
# --- Global Utility Functions (specific to BO) ---

# Custom Hamming Distance Kernel for DiceKriging
...
# Covariance function for DiceKriging based on Hamming distance.
# This defines the similarity between two points (binary rule vectors)
...
# Calculates the Expected Improvement (EI) for candidate points.
# EI balances exploration (sampling in uncertain regions) and exploitation (sampling near current best points).
expected_improvement_r <- function(X_candidate, gp_model, X_sample, Y_sample, xi) { # xi is now a direct parameter
...
# It uses a hybrid strategy: searching around cluster representatives (exploitation)
# and performing random search in the full binary space (exploration).
optimize_acquisition_function_r <- function(acquisition_func, gp_model, X_sample, Y_sample, n_atomic_rules_bo,
                                            cluster_representatives = NULL, n_random_search = 1000,
                                            max_active_rules_acquisition = 3, xi_val) { # xi_val passed here
  ...
  # Strategy 1: Search around cluster representatives (Exploitation/Refinement)
  # This focuses the search on areas near previously high-performing solutions.
  if (!is.null(cluster_representatives) && length(cluster_representatives) > 0) {
    for (rep_vector_list in cluster_representatives) { # Each rep_vector_list is a list containing a single row matrix
      rep_vector <- as.numeric(rep_vector_list[[1]]) # Extract the vector from the list and matrix
      
      neighbors <- list()
      
      # Generate Hamming Distance 1 neighbors (flipping one bit)
      for (i in 1:n_atomic_rules_bo) {
        neighbor_hd1 <- rep_vector
        neighbor_hd1[i] <- 1 - neighbor_hd1[i]
        neighbors[[length(neighbors) + 1]] <- neighbor_hd1
      }
      
      # Generate Hamming Distance 2 neighbors (flipping two bits), limited for large dimensions
      if (n_atomic_rules_bo <= 20) { # Limit to avoid combinatorial explosion
        for (i in 1:n_atomic_rules_bo) {
          temp_hd1_vec = rep_vector
          temp_hd1_vec[i] = 1 - temp_hd1_vec[i]
          for (j in (i + 1):n_atomic_rules_bo) {
            neighbor_hd2 = temp_hd1_vec
            neighbor_hd2[j] = 1 - neighbor_hd2[j]
            neighbors[[length(neighbors) + 1]] <- neighbor_hd2
          }
        }
      }
      
      # Add the representative itself to candidates
      neighbors[[length(neighbors) + 1]] <- rep_vector
      
      neighbors_matrix <- do.call(rbind, neighbors)
      neighbors_matrix <- unique(neighbors_matrix) # Remove duplicates
      
      # Filter out candidates already sampled
      neighbors_to_check <- matrix(nrow = 0, ncol = n_atomic_rules_bo)
      if (nrow(neighbors_matrix) > 0) {
        for (k in 1:nrow(neighbors_matrix)) {
          current_neighbor <- neighbors_matrix[k, ]
          if (!any(apply(X_sample, 1, function(row) all(row == current_neighbor)))) { # Check if already sampled
            neighbors_to_check <- rbind(neighbors_to_check, current_neighbor)
          }
        }
      }
      
      # Evaluate EI for new neighbors and update best_x if a better candidate is found
      if (nrow(neighbors_to_check) > 0) {
        ei_values <- acquisition_func(neighbors_to_check, gp_model, X_sample, Y_sample, xi = xi_val) # Pass xi_val
        max_ei_idx <- which.max(ei_values)
        if (ei_values[max_ei_idx] > best_ei) {
          best_ei <- ei_values[max_ei_idx]
          best_x <- neighbors_to_check[max_ei_idx, ]
        }
      }
    }
  }
  
  # Strategy 2: Random search in the full binary space (Exploration)
  ...
  if (nrow(random_vectors_to_check) > 0) {
    ei_values_random <- acquisition_func(random_vectors_to_check, gp_model, X_sample, Y_sample, xi = xi_val) # Pass xi_val
...
# --- Function: Generate Atomic Rules ---
# This function systematically generates all possible "atomic" rule predicates
# from the provided numeric and categorical clinical metrics.
# These atomic rules form the basis of the binary search space for BO.
generate_all_atomic_rules <- function(de_data, numeric_metrics, categorical_metrics) {
  atomic_rules_list <- list() # Stores the string representation of each atomic rule
  atomic_rules_map <- list() # Maps rule strings to their binary index
  current_binary_idx <- 0 # Counter for assigning unique binary indices
  
  # Generate rules for numeric metrics (e.g., "Metric > K" and "Metric <= K")
  for (metric in numeric_metrics) {
    min_val <- floor(min(de_data[[metric]], na.rm = TRUE))
    max_val <- ceiling(max(de_data[[metric]], na.rm = TRUE))
    
    # Skip if the metric has an invalid range
    ...
    # Create rules for each integer value within the metric's range
  ...
  # Generate rules for categorical metrics (e.g., "Metric == 'Level'")
...
# --- Function: Objective Function for BO (evaluate_subgroup_fitness_bo) ---
# This function evaluates the "fitness" of a patient subgroup defined by a binary rule vector.
# Fitness is based on the subgroup's mean protein expression relative to healthy volunteers,
# with penalties for small subgroups or invalid results.
evaluate_subgroup_fitness_bo <- function(binary_rule_vector, de_data, hv_aval_mean, atomic_rules_list, protein_variable_name, min_subgroup_size) {
  binary_rule_vector <- as.logical(binary_rule_vector) # Convert 0/1 to TRUE/FALSE
  active_predicate_strings <- atomic_rules_list[binary_rule_vector] # Get active rule strings
  
  # If no rules are active, return a very small positive fitness to avoid errors
  if (length(active_predicate_strings) == 0) {
    return(1e-9)
  }
  
  filter_expressions <- list()
  # Parse each active predicate string into a dplyr filter expression
  for (pred_str in active_predicate_strings) {
    if (grepl(" %in% ", pred_str)) { # Categorical levels (e.g., "Metric %in% c('A', 'B')")
      parts <- strsplit(pred_str, " %in% ")[[1]]
      metric_name <- trimws(parts[1])
      levels_str_raw <- trimws(parts[2])
      levels_parsed <- eval(parse(text = levels_str_raw)) # Safely parse string to R vector
      filter_expressions[[length(filter_expressions) + 1]] <- rlang::expr(!!sym(metric_name) %in% !!levels_parsed)
    } else if (grepl(" <= ", pred_str)) { # Numeric less than or equal (e.g., "Metric <= 10")
    ...
    } else if (grepl(" > ", pred_str)) { # Numeric greater than (e.g., "Metric > 5")
      parts <- strsplit(pred_str, " > ")[[1]]
      metric_name <- trimws(parts[1])
      val <- as.numeric(trimws(parts[2]))
      filter_expressions[[length(filter_expressions) + 1]] <- rlang::expr(!!sym(metric_name) > !!val)
    } else if (grepl(" == ", pred_str)) { # Categorical exact match (e.g., "Metric == 'Level'")
      parts <- strsplit(pred_str, " == ")[[1]]
      metric_name <- trimws(parts[1])
      level_val <- gsub("'", "", trimws(parts[2])) # Remove quotes
      ...
      return(1e-9) # Return low fitness for unrecognized formats
    }
  }
  
  # Apply the generated filters sequentially to the disease patient data
  filtered_de_data <- de_data
  for (filter_expr in filter_expressions) {
    filtered_de_data <- filtered_de_data %>% filter(!!filter_expr)
    # If subgroup size drops below minimum at any step, return low fitness
...
# --- Function: Main Bayesian Optimization Loop Function for a single min_subgroup_size ---
# This function encapsulates the core Bayesian Optimization process for a fixed minimum subgroup size.
run_bayesian_optimization_for_min_size <- function(n_atomic_rules_bo, initial_samples, bo_iterations,
                                                   epsilon_clustering, min_samples_clustering,
                                                   bo_acquisition_random_search, max_initial_active_rules,
                                                   de_data, hv_aval_mean, atomic_rules_list, protein_variable_name,
                                                   min_subgroup_size_fixed, xi) { # xi is now passed to this function
  
  X_observed_r <- matrix(nrow = 0, ncol = n_atomic_rules_bo) # Stores observed rule vectors
  Y_observed_r <- numeric(0) # Stores observed fitness values
  
  best_overall_y <- -Inf # Tracks the best fitness found so far
  best_overall_x <- NULL # Stores the rule vector corresponding to the best fitness
  
  # Phase 1: Initial Sampling and Evaluation
  # Randomly sample and evaluate a set of initial points to seed the Gaussian Process model.
  for (i in 1:initial_samples) {
    # Generate a random binary rule vector with a limited number of active rules
    ...
    # Evaluate the fitness of the generated rule vector
  ...
  # Iteratively train the GP, optimize acquisition, and evaluate new points.
  for (i in 1:bo_iterations) {
    # Step 1: Empirical Equivalence Class Discovery (Clustering on Y_observed_r)
    # Use DBSCAN to find clusters of similar fitness values, guiding exploitation.
    y_for_clustering <- as.matrix(Y_observed_r)
    dbscan_result <- dbscan::dbscan(y_for_clustering, eps = epsilon_clustering, minPts = min_samples_clustering)
    cluster_labels <- dbscan_result$cluster
    
    cluster_representatives_r <- list()
    unique_clusters <- unique(cluster_labels)
    
    # Select the best performing point from each identified cluster as a representative
    if (length(unique_clusters) > 1 || (length(unique_clusters) == 1 && unique_clusters[1] != 0)) {
      if (0 %in% unique_clusters) { # Cluster 0 usually represents noise points in DBSCAN
        unique_clusters <- unique_clusters[unique_clusters != 0]
      }
      
      for (cluster_id in unique_clusters) {
        cluster_indices <- which(cluster_labels == cluster_id)
        best_in_cluster_idx <- cluster_indices[which.max(Y_observed_r[cluster_indices])]
        cluster_representatives_r[[length(cluster_representatives_r) + 1]] <- X_observed_r[best_in_cluster_idx, , drop = FALSE]
      }
    }
    
    # Step 2: Train Gaussian Process Surrogate Model (DiceKriging)
    # The GP model approximates the unknown objective function.
    cov_type_ham <- "custom"
    ...
    # Step 3: Optimize Acquisition Function to find the next point to sample
    if (is.null(km_model)) {
      ...
      next_x_r <- optimize_acquisition_function_r(expected_improvement_r, km_model, X_observed_r, Y_observed_r,
                                                  n_atomic_rules_bo, cluster_representatives_r,
                                                  bo_acquisition_random_search, max_initial_active_rules, xi_val = xi) # Pass xi
    }
    
    # Step 4: Evaluate the new point and update observed data
...
# --- Function: Orchestrate Parallel BO Runs ---
# Manages the execution of multiple Bayesian Optimization runs in parallel,
...
orchestrate_parallel_bo <- function(min_subgroup_sizes_to_test, params, N_ATOMIC_RULES_BO,
  ...
  if (!isTRUE(params$benchmark_mode)) {
    message("\n--- Running Bayesian Optimization for different min_subgroup_size values (in parallel) ---\n")
  }
  ...
  clusterExport(cl_outer, c(
    ...
    "N_ATOMIC_RULES_BO", "params" # params now contains xi
  ...
  all_results_bo_raw <- foreach(
    ...
    result <- run_bayesian_optimization_for_min_size(
      ...
      epsilon_clustering = params$bo_epsilon_clustering,
      min_samples_clustering = params$bo_min_samples_clustering,
      ...
      xi = params$bo_xi # Pass xi parameter to the BO function
    )
    return(result)
  }
...
# --- Function: Process BO Results into Detailed DataFrames ---
# This function decodes the optimal binary rule vectors from the BO runs
  ...
  if (length(valid_results_bo) > 0) {
    for (i in seq_along(valid_results_bo)) {
      ...
      for (j in 1:length(atomic_rules_list)) {
        if (current_best_chromosome_bo[j] == 1) { # If the j-th atomic rule is active
          pred_str <- atomic_rules_list[[j]]
          
          # Parse the predicate string into an R expression and add to filter list
          if (grepl(" %in% ", pred_str)) {
            ...
            # For rules table: store levels as comma-separated string
            ...
            parts <- strsplit(pred_str, " == ")[[1]]
            metric_name <- trimws(parts[1])
            level_val <- trimws(parts[2]) # Keep quotes for now, strip later for display
            final_filter_expressions_apply_i[[length(final_filter_expressions_apply_i) + 1]] <- rlang::expr(!!sym(metric_name) == !!gsub("'", "", level_val)) # Strip quotes for R expression
            
            value_for_table <- level_val # Store with quotes for now
            operator_for_table <- "=="
            
          } else {
            warning(paste("Unrecognized predicate format:", pred_str))
            next # Skip this rule if format is unrecognized
          }
          
          # Add the decoded rule to the temporary rules table for the current solution
          rules_table_for_current_solution <- rbind(rules_table_for_current_solution, data.frame(
            Metric = metric_name, # Directly use metric_name parsed from pred_str
...
# --- Main Orchestrating Function for BO ---
# This is the main function to run the Bayesian Optimization patient subgroup discovery process.
# It orchestrates rule generation, parallel BO runs, results processing, and visualization.
run_bayesian_optimization_analysis_orbit <- function(params, df, dfThisProtein, hv_data, de_data, hv_aval_mean, cl_outer = NULL) {
  
  # 1. Define Atomic Rules: Generate all possible single predicates.
  ...
  atomic_rules_map <- atomic_rules_info$atomic_rules_map # Not directly used downstream in functions, but good to keep
  N_ATOMIC_RULES_BO <- atomic_rules_info$n_atomic_rules_total # Total number of atomic rules (dimension of binary space)
  
  all_results_bo_raw <- list()
  
  # 2. Run or Load Bayesian Optimization Results
  if (params$runSim) {
    # If runSim is TRUE, execute the BO simulation in parallel.
    ...
    # If runSim is FALSE, attempt to load previously saved BO results.
    if (!isTRUE(params$benchmark_mode)) {
    ...
    tryCatch({
      ...
      # This requires defining a dummy N_ATOMIC_RULES_BO if it wasn't set due to data loading failure.
      if (!exists("N_ATOMIC_RULES_BO") || is.null(N_ATOMIC_RULES_BO)) {
        N_ATOMIC_RULES_BO_dummy <- 10 # A small default if atomic rules couldn't be generated
      } else {
  ...
  # Stop execution if no BO results are available after simulation or loading.
  ...
  # 3. Process BO Results into Detailed DataFrames
  # This function decodes the optimal binary chromosomes into human-readable rules
  # and calculates various statistics for each identified subgroup.
  ...
  # best fitness achieved by the BO.
  if (!isTRUE(params$benchmark_mode)) {
